...
You may also have noticed the %\>% symbol, or the pipe, above. For now,
you can read this as telling R, "then, ...". So, you can read the above
as starting with `d`, your data, then selecting the three named
variables. We'll be sure to explore the pipe in more detail in the first
module.
...
(measured through their learning management system in hours,
`TimeSpent_hours`.
...
There's a lot here in the output. For now, if you're new to regression
models and modeling, the values in the Estimate column are perhaps the
most important; those represent the intercept (for the row with labeled
(Intercept) and slopes (for all of the other rows) for your model.

Note that the estimate for `TimeSpent_hours` is 0.36. We can interpret
this as telling us that for every additional hour students spend on the
course, the estimated value for their final grade will be 0.36 (on a
0-100 scale) greater than the intercept, which is around 65. Thus, if a
student spent, for instance, 40 hours on the course, their estimated
final grade would be 0.36 X 40, or around 15, plus 65, for an estimate
of around 80, a grade right on the cusp of a B.

If you have a lot of questions, then you're in the right place! We'll
say a lot more about this, too, in later learning labs.
...
web browser and shared online. You can view this file any time by
clicking the name of the file where it appears in the file viewer in the
pain on the right; after clicking on the name of the file, you should
see an option to "View in Web Browser."
