...
```{r}
m2 <- lm(proportion_earned ~ int + time_spent_hours + gender + val + pc, data = data_to_explore)
summary(m2)
```
...
```{r}
# the * operator specifies an interaction
m3 <- lm(proportion_earned ~ int + time_spent_hours + gender + time_spent_hours*gender, data = data_to_explore)
summary(m3)
```
