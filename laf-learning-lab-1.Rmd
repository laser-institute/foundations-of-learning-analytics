...
1.  Joining together different data sets
...
-   Finally, a picture to your slack account (from computer): Please go
    to the left side of your slack account and click on your name. Then
    select 'profile and account'. Go to the 'edit profile' (on the right
    side of your workspace) and upload an image.
...
```{r}
time_spent_not_cleaned <- read_csv(here("data", "log.csv"))

time_spent <- read_csv(here("data", "log.csv")) %>% 
  clean_names()
```

What is different about log_data_not_cleaned and log_data? Type those
two names in the console, below, to view a summary of what data two
names point to. Then, add a thought or two below after the bullet point
explaining what, precisely, differs about the two sets of names:
...
2.  Calculating the percentage of points students earned
...
often helpful t take a look at the data before writing code.

Below, we will load `time_spent` and run the `separate()` function to
...
You may also wish to change the color; reflect back to the getting started task for an 
...
If you wanted, you could save the processed data set to your data folder. The 
`write_csv()` function is useful for this. The following code is set to not run, as we
wanted to ensure that everyone had the data set needed to begin the second learning 
lab, but if you're confident in your prepared data, you can save it with the following:
